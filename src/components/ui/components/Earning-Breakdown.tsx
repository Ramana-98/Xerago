...
const EarningBreakdown = forwardRef<HTMLDivElement, EarningBreakdownProps>(
  ({ highlight = false }, ref) => {
    ...
    return (
      <Card
        ref={ref}
        className={`rounded-2xl shadow w-full max-w-md md:max-w-none lg:max-w-md mx-auto p-5 transition-all duration-200 hover:bg-gray-300 hover:-translate-y-2 ${
      ...
      >
        <CardHeader className="pb-1">
          <CardTitle className="text-2xl md:text-3xl lg:text-2xl font-bold">Earnings Breakdown</CardTitle>
        </CardHeader>
        <CardContent >
          <Tabs value={tab} onValueChange={setTab} className="mb-2">
            <TabsList className="bg-gray-100 rounded-lg p-1 inline-flex mb-7 flex justify-center w-full md:w-auto lg:w-auto md:mx-auto">
              <TabsTrigger
                value="week"
                className={
                  tab === "week"
                    ? "bg-white shadow text-black font-semibold px-4 py-1 md:px-6 md:py-2 lg:px-4 lg:py-1 rounded-lg md:text-base lg:text-sm"
                    : "text-gray-500 px-4 py-1 md:px-6 md:py-2 lg:px-4 lg:py-1 md:text-base lg:text-sm"
                }
              >
                This week
              </TabsTrigger>
          ...
          <div className="flex flex-col sm:flex-row md:flex-row lg:flex-row items-center gap-4">
            <div className="flex-1 flex flex-col items-start md:items-center lg:items-start">
              <div className="text-3xl md:text-4xl lg:text-3xl font-extrabold mb-1">${data.total.toLocaleString()}</div>
              <div className="text-gray-500 text-base md:text-lg lg:text-base mb-4">Total Earnings</div>
              <div className="flex flex-col gap-2 mt-2 md:gap-3 lg:gap-2">
                <div className="flex items-center gap-2 md:gap-3 lg:gap-2">
                  ...
                  ></div>
                  <span className="font-bold md:text-lg lg:text-base">Paid</span>
                  <span className="ml-4 text-base md:text-lg lg:text-base">${data.paid.toLocaleString()}</span>
                </div>
                <div className="flex items-center gap-2 md:gap-3 lg:gap-2">
                  <div 
                    className="w-3 h-3 md:w-4 md:h-4 lg:w-3 lg:h-3 rounded-full" 
                  ...
                  ></div>
                  <span className="font-bold md:text-lg lg:text-base">Pending</span>
                  <span className="ml-4 text-base md:text-lg lg:text-base">${data.pending.toLocaleString()}</span>
                </div>
              </div>
            </div>
